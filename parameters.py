...
#matches = matches[0:2000]

clubs = pd.read_csv('./data/spi_global_rankings.csv')
clubs.set_index('name')
#clubs.loc[clubs['name'] == 'Manchester United', 'spi'].iloc[0]
...
def minusloglik(x0):
    ...
    for i in range(len(matches)):
        ...
        except: # some of the data rows raises an error, dunno why but i exclude them like this
            pass
    return out
...
#res = minimize(loglik, x0) # 0.4018, 0.12033, 1.0478
